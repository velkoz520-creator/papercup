...
const CLIENT_BUILD = "20260915-ui2";
window.__BUILD_TAG = CLIENT_BUILD;   // ?demo=1 预览模式读这个

// ── 设备执行器（A 路回环桥，09-10 接线）────────────────────────
// 服务端 device 消息 → 本机设备服务（同机回环，独立体检页同款通路）。
// cors 优先（可读回包），失败回落 no-cors（发射后不管）。
const DEVICE_URL = 'http://127.0.0.1:20010/command';
const DEVICE_STOP_ON_CLOSE = true;   // 断线/挂断自动急停（工程卫生默认，非限制）

function devicePayloads(cmd) {
  const base = { toy: '', apiVer: 1, stopPrevious: 0 };
  switch (cmd && cmd.action) {
    case 'vibrate': return [{ command: 'Function', action: `Vibrate:${cmd.strength}`, timeSec: cmd.timeSec, ...base }];
    case 'rotate':  return [{ command: 'Function', action: `Rotate:${cmd.strength}`, timeSec: cmd.timeSec, ...base }];
    case 'combo':   // 震+旋两条并发，stopPrevious:0 让双通道共存（橘瓣实战写法）
      return [{ command: 'Function', action: `Vibrate:${cmd.vibrate}`, timeSec: cmd.timeSec, ...base },
              { command: 'Function', action: `Rotate:${cmd.rotate}`, timeSec: cmd.timeSec, ...base }];
    case 'hold':    // 悬停循环：app 服务端 run/pause 交替
      return [{ command: 'Function', action: `Vibrate:${cmd.vibrate}`, timeSec: cmd.timeSec, loopRunningSec: cmd.runSec, loopPauseSec: cmd.gapSec, ...base },
              { command: 'Function', action: `Rotate:${cmd.rotate}`, timeSec: cmd.timeSec, loopRunningSec: cmd.runSec, loopPauseSec: cmd.gapSec, ...base }];
    case 'tide':    return [{ command: 'Pattern', rule: 'V:1;F:vrp;S:100#', strength: (cmd.strengths || []).join(';'), timeSec: cmd.timeSec, ...base }];
    case 'stock':   return [{ command: 'Preset', name: cmd.name, timeSec: cmd.timeSec, ...base }];
    case 'stop':    return [{ command: 'Function', action: 'Stop', timeSec: 0, ...base }];
    default: return [];
  }
}

async function _devicePost(payload) {
  try {
    await fetch(DEVICE_URL, { method: 'POST', mode: 'cors',
      headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
  } catch (e) {
    try {  // cors 意外被拦/偶发无应答 → no-cors 盲发（发射后不管）
      await fetch(DEVICE_URL, { method: 'POST', mode: 'no-cors',
        headers: { 'Content-Type': 'text/plain' }, body: JSON.stringify(payload) });
    } catch (_) { /* 本机链路不在：静默，通话本身不受影响 */ }
  }
}

async function deviceHandle(cmd) {
  const payloads = devicePayloads(cmd);
  if (!payloads.length) return;
  await Promise.all(payloads.map(_devicePost));
}

function deviceStop() { if (DEVICE_STOP_ON_CLOSE) deviceHandle({ action: 'stop' }); }
...
export class VoiceCall {
  ...
  async start() {
    ...
    await this._connect();
    // 前端版本上报：手机浏览器缓存可能滞留旧 JS，服务端日志可查她实际拿到的版本
    this._send({ type: 'start', video: this.wantVideo, sample_rate: 16000, client_build: CLIENT_BUILD });
    this._clientBuild = CLIENT_BUILD;   // 拨号页显示用
    console.log('[voice-call] build', CLIENT_BUILD);
  ...
  _connect() {
    return new Promise((resolve, reject) => {
      ...
      ws.onclose = (e) => { this.ws = null; deviceStop(); if (this.mode !== 'idle') { this._setMode('idle'); this.emit('closed', e); } };
  ...
  async _onMessage(data) {
    ...
    switch (msg.type) {
      ...
      case 'device': deviceHandle(msg.cmd).catch(() => {}); break;
  ...
  _drain() {
    ...
    while (this._queue.length) {
      ...
      src.onended = () => {
        ...
        if (!this._sources.length && this.playing) {
          // 闻序小尾巴：playing=实际扬声器状态，段间空隙立即复位——
          // 音量显示与打断门槛（barge 判定用 playing）不等整轮完成
