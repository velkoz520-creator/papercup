...
try:
    import device                  # 设备标记层（A 路回环桥）。发布隔离：公开快照无此模块，
except ImportError:                # 公开面读到的是无设备版（server 功能不受影响）
    device = None
import time
import uuid
import wave
...
FAREWELL_RE = re.compile(
    ...
    r"|\bbyebye\b|bye\s?bye|goodbye|\bbye\b|\bnight\s?night\b|good\s?night)")
# 反例保护："别挂/不许挂"含"挂"字绝不能当告别；"by the way"被 ASR 转成"bye the way"不能当 bye。
# 注意"不聊了/不说了"故意不收——话题转换也这么说，误挂比漏挂（她手动挂）事故得多
FAREWELL_NEG_RE = re.compile(r"(别挂|不许挂|不准挂|不要挂|不能挂|谁挂|还没挂|没挂|by\s?the\s?way|bye\s?the\s?way)")

# V2 流式 TTS 总开关（B6 三保险精神）：SSE 增量→切句→逐段合成下发，首句不再等全量。
# 出问题 env 置 0 秒回整段老路（代码保留原路径为回落）。
STREAM_TTS = os.getenv("PAIVOICE_STREAM_TTS", "1") == "1"

_gw_sem = asyncio.Semaphore(1)  # 同一时刻只投递一轮，避免两句转录并发进网关
...
async def _call_gateway(http: aiohttp.ClientSession, turn: dict, metrics: dict | None = None,
                        on_segment=None, on_device=None) -> str:
    ...
    payload = {
        ...
        "stream": True,
        "max_tokens": 600,
        "client": "papercup",   # 网关据此注入设备说明书（橘瓣通话不带）
    }
    parts: list[str] = []
    first_at = None
    ...
    device_filter = (device.DeviceMarkerFilter()
                     if on_device is not None and device is not None else None)
    ...
    async with http.post(GATEWAY_URL, json=payload, headers=headers,
        ...
        async for raw in response.content:
            ...
            if piece:
                ...
                if device_filter is not None:      # 设备标记剥在切句之前：字幕/TTS/归档三处都看不到
                    clean, dev_cmds = device_filter.feed(clean)
                    if dev_cmds:
                        await on_device(dev_cmds)
                if not clean:
                    continue
                parts.append(clean)
        ...
        if device_filter is not None and tail_text:
            tail_text, dev_cmds = device_filter.feed(tail_text)
            if dev_cmds:
                await on_device(dev_cmds)
        if tail_text:
...
async def request_reply(http: aiohttp.ClientSession, turn: dict, metrics: dict | None = None,
                        on_segment=None, on_device=None) -> str:
    """路由：配置了 GATEWAY_URL 走网关快车道，否则返回 mock 兜底文案。"""
    if GATEWAY_URL:
        async with _gw_sem:
            return await _call_gateway(http, turn, metrics, on_segment=on_segment,
                                       on_device=on_device)

    # GATEWAY_URL 未配置时 mock 兜底（原上游 tmux Adapter 兼容路线已随其移除）
    return f"我听见了：{turn['transcript']}" if turn["transcript"] else "我没有听清楚。"


# ── 设备控制指令调度（A 路回环桥，09-10 接线）────────────────────────
# 标记在网关流里被 device.DeviceMarkerFilter 剥除后到这里：
#   - 即时类（vib/rot/combo/hold/tide/stock/stop）直接下发 WS，前端转译打本机 Game Mode
#   - 场景类（tease/edge/flood）挂后台任务按时序下发；pulse 常驻循环
#   - 新指令自动接管（取消旧排程）；挂断/断线由 session finally 收割（all_tasks 注册）
# 安全姿态（天天 09-10 拍板）：无封顶无限时；指令自带 timeSec 自然到期；断线前端自停。

import random as _random   # 场景/脉冲的随机时长（人类刻度，勿回退工程师秒表）


async def _device_scene(ws, steps: list) -> None:
    try:
        for step in steps:
            if "wait" in step:
                await asyncio.sleep(step["wait"])
            else:
                await send(ws, {"type": "device", "cmd": step["cmd"]})
    except asyncio.CancelledError:
        raise
    except Exception as e:
        print(f"[device] scene aborted: {e}", flush=True)


async def _device_pulse(ws, params: dict) -> None:
    """常驻脉冲（K 09-10 亲笔：满档 combo 1~2s → 随机歇 3~8s，无限循环）。
    终止 = (stop)/新指令接管（_device_handle 取消旧任务）/挂断收割。"""
    try:
        while True:
            burst = _random.randint(params["burstSec"][0], params["burstSec"][1])
            await send(ws, {"type": "device", "cmd": {
                "action": "combo", "vibrate": params["strength"],
                "rotate": params["rotate"], "timeSec": burst}})
            await asyncio.sleep(_random.uniform(*params["gapSec"]))
    except asyncio.CancelledError:
        raise
    except Exception as e:
        print(f"[device] pulse aborted: {e}", flush=True)


async def _device_handle(ws, call, cmd: dict) -> None:
    action = cmd.get("action")
    old = getattr(call, "device_task", None)
    if old and not old.done():
        old.cancel()                     # 新指令接管：旧场景/常驻排程即废（在跑的指令 timeSec 自然到期）
    call.device_task = None
    if action == "scene":
        name = cmd.get("name", "")
        steps = device.expand_scene(name)
        if name == "pulse":
            t = _register_task(call, _device_pulse(ws, steps[0]["cmd"]))
        else:
            t = _register_task(call, _device_scene(ws, steps))
        if t is not None:
            call.device_task = t
        return
    await send(ws, {"type": "device", "cmd": cmd})
...
async def answer_turn(ws, call: Call, http: aiohttp.ClientSession, pcm: bytes,
    ...
    try:
        ...
        if streamed:
            ...
            async def _emit_device(dev_cmds: list) -> None:
                for cmd in dev_cmds:
                    await _device_handle(ws, call, cmd)
            ...
            try:
                reply = await request_reply(http, {"call_session_id": call.id, "turn_id": turn_id,
                                            ...
                                            on_device=_emit_device if device is not None else None)
        ...
        if device is not None and not streamed and reply:
            # 非流式路径（网关未配置/流式关）：整段过一遍设备标记再走 TTS
            _df = device.DeviceMarkerFilter()
            _, _dev_cmds = _df.feed(reply)
            _tail, _dev_tail = _df.flush()
            _dev_cmds += _dev_tail
            if _dev_cmds:
                for _cmd in _dev_cmds:
                    await _device_handle(ws, call, _cmd)
        if not streamed:
            ...
            async for raw in ws:
                ...
                if kind == "start":
                    if TOKEN and not token_ok and event.get("token") != TOKEN:
                        ...
                        return
                    token_ok = True
                    cb = event.get("client_build")
                    if cb:
                        print(f"[session] client_build={cb}", flush=True)   # 她手机实际拿到的前端版本
...
async def main() -> None:
    ...
    # 链路体检页（09-09 临时诊断：设备回环验证用独立页，文件缺失时路由自动不出现）
    tt_paths = [Path(__file__).parent / "toy-test.html",
                Path(__file__).parent.parent / "web-client" / "toy-test.html"]
    tt_text = next((p.read_text(encoding="utf-8") for p in tt_paths if p.exists()), "")
    if tt_text:
        static_files["/toy-test.html"] = ("text/html; charset=utf-8", tt_text)

    # UI v2 静态资产（09-15）：orb 动画模块、旧版逃生门、fonts/skins/sprites 目录
    def _first(paths, default):
        return next((p for p in paths if p.exists()), None)
    orb_paths = [Path(__file__).parent / "orb.js",
                 Path(__file__).parent.parent / "web-client" / "orb.js"]
    orb_file = _first(orb_paths, None)
    legacy_paths = [Path(__file__).parent / "legacy.html",
                    Path(__file__).parent.parent / "web-client" / "legacy.html"]
    legacy_file = _first(legacy_paths, None)
    asset_root = _first([Path(__file__).parent / "assets",
                         Path(__file__).parent.parent / "web-client" / "assets"], None)
    asset_root = asset_root.resolve() if asset_root else None
    asset_types = {".webp": "image/webp", ".png": "image/png", ".jpg": "image/jpeg",
                   ".woff2": "font/woff2", ".js": "application/javascript",
                   ".css": "text/css", ".svg": "image/svg+xml"}
    from websockets.datastructures import Headers as _H
    from websockets.http11 import Response as _R

    def _file_response(body: bytes, ctype: str, cacheable: bool = False):
        headers = [("content-type", ctype),
                   ("content-length", str(len(body))),
                   ("access-control-allow-origin", "*")]
        # 资产不加长缓存：文件名不随版本变，强缓存会复刻"手机滞留旧 JS"案
        if cacheable:
            headers.append(("cache-control", "public, max-age=3600"))
        else:
            headers.append(("cache-control", "no-cache"))
        return _R(200, "OK", _H(headers), body)
    ...
    def process_request(connection, request):  # 静态托管通话页 + voice-call.js；其余路径走 WS 升级
        ...
        try:
            ...
            if path == "/orb.js" and orb_file:
                return _file_response(orb_file.read_bytes(), "application/javascript")
            if path == "/legacy.html" and legacy_file:
                return _file_response(legacy_file.read_text(encoding="utf-8").encode("utf-8"),
                                      "text/html; charset=utf-8")
            if path.startswith("/assets/") and asset_root:
                target = (asset_root / path[len("/assets/"):]).resolve()
                if (str(target).startswith(str(asset_root)) and target.is_file()
                        and target.suffix.lower() in asset_types):
                    return _file_response(target.read_bytes(), asset_types[target.suffix.lower()],
                                          cacheable=True)
