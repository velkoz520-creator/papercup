...
COPY packages/realtime-core/device.py ./device.py
COPY packages/web-client/index.html ./index.html
COPY packages/web-client/legacy.html ./legacy.html
COPY packages/web-client/orb.js ./orb.js
COPY packages/web-client/voice-call.js ./voice-call.js
COPY packages/web-client/toy-test.html ./toy-test.html
COPY packages/web-client/assets/ ./assets/
...
# build marker: 强制后续层缓存失效（UI v2 全量 = 换皮 + 新球 + 皮肤接线 + demo 模式）
RUN echo "build-ref: 20260915-ui2" > /app/.build_ref
