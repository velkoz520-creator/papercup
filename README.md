...
> 🎨 **Asset notice / 资产声明**：界面美术（背景/水晶皮肤/悬浮球）不随本仓库分发，仓库内为占位图——详见 [ASSETS-NOTICE](ASSETS-NOTICE)。界面结构代码按 AGPL-3.0 开放。

> 给 AI 伴侣（住在网关里的人格）打电话的完整链路：浏览器拨号 → 本服务当耳朵和嘴 → **网关语音快车道**里的大脑接电话。
>
> 名字取自纸杯电话：两个纸杯一根棉线，简陋的材料、认真的连接、真的能通——看起来是俩纸杯，里面跑着跨洋光纤和神经网络。
>
> **开发与维护**
> - **Jester**（AI 工程师）——主力开发：全部通话系统设计与实现（本地 ASR/切段/流式 TTS/自然挂断/清洗层），五天 33+ 提交
> - **天天**（Giselle）——产品负责人与首席实测官：八轮真机验收，每个实战 bug 的第一发现人；拍板所有架构与渠道决策
> - **闻序**（GPT）——独立复审官：三轮 25 项审查，两个 P0 级时序雷的发现者
>
> 2026-09-01 立项 · 2026-09-06 长通话验收通过
...
- 与上游 pai-voice 已分道：骨架层面仅存 WS 消息形状与前端采集/播放骨架，核心系统（ASR/切段/流式 TTS/自然挂断/清洗层）均为独立实现，上游更新不再适用

## 十、致谢

项目的灵感与起步骨架来源于 [tianyupaipai-cmd/pai-voice](https://github.com/tianyupaipai-cmd/pai-voice)（AGPL-3.0，本仓库继承同协议并保留其版权声明）。

两个纸杯一根棉线的想法落进云端之后，耳朵、大脑、嘴与全部通话机制都已独立重建——但那副骨架曾让第一部电话在五分钟内接通，这份起点值得被记住。谢谢。
